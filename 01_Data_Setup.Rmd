---
title: "Data Preparation - Unemployment First Law Introduciton"
...
# Introduction of unemployment data, use the work-injury template for conversion
...
# Inter-State War CoW - v4.0
war <- read.csv("data/Cow Interstate Wars.csv", header = T, sep = ";") # CoW data

# Intra-State War Cow - v5.1
war_intra <- read_csv(file = "data/INTRA-STATE_State_participants v5.1 CSV.csv")

# too much missing data on deaths, can't use

war_intra <- select(war_intra, CcodeA, SideA, StartYr1, EndYr1, `Deaths A`, `Deaths B`)


# Recode data into long

war_intra <- war_intra %>%
  mutate(SideA = car::recode(SideA, recodes = "'Ottoman Empire' = 'Turkey'; 'Prussia' = 'Germany'; 'Sardinia/Piedmont' = 'Italy'; 'Two Sicilies' = 'Italy'; 'Sardinia' = 'Italy'; 'Damascus and Aleppo' = 'Syria'; 'Maronites' = 'Lebanon'; 'Yucatan' = 'Mexico'; 'Kashgarians' = 'China'; 'Sichuan' = 'China'; 'An-Fu Faction' = 'China'; 'Soviet Russian Republic' = 'Russia'; 'Blackshirts' = 'Italy'; 'Golok Tibetans' = 'China'; 'Guominjun' = 'China'; 'GMD' = 'China'; 'Xinjiang' = 'China'; 'USSR' = 'Russia'; 'Hui' = 'China'; 'Egypt/United Arab Republic' = 'Egypt'; 'Regional Militaries' = 'China'; 'ELF'= 'Eritrea'; 'Maronite militias' = 'Lebanon'; 'Rhodesia' = 'Zimbabwe'; 'Lebanese Forces' = 'Lebanon'; 'Kampuchea' = 'Vietnam'; 'SPLA-Garang' = 'South Sudan'; 'SPLA-Nasir and Anya Nya II' = 'Sudan'; 'Tur Loyalists' = 'Somalia'; 'Somaliland' = 'Somalia'; 'Freikorps' = 'Germany'; 'Inkatha' = 'South Africa'; 'Jukun' = 'South Sudan'; 'Kashgar' = 'China'; 'Minnesota' = 'United States of America'; 'Houthi Rebels' = 'Yemen';  'Banyarwanda Militia' = 'Democratic Republic of Congo'; 'PUK' = 'Iraq'; 'Laskar Jihad' = 'Indonesia'; 'Christian Tarok' = 'Nigeria'; 'Islamic State' = 'Syria'; 'Jabhat al-Nusra' = 'Syria'; 'Ansar Allah' = 'Yemen'; 'Al-Shabaab' = 'Somalia'; 'Libyan Dawn' = 'Libya'"),
    cow_code = countrycode(SideA, "country.name", "cown"),
    cow_code = ifelse(is.na(cow_code), CcodeA, cow_code),
    BatDeath = `Deaths A` + `Deaths B`,
    year = StartYr1,
    EndYr1 = ifelse(EndYr1 == -9 | EndYr1 > 2010, 2010, EndYr1)
         ) %>%
  select(cow_code, year, StartYr1, EndYr1)

war <- war %>%
  mutate(StateName = ifelse(StateName == "Ottoman Empire", "Turkey", ifelse(StateName == "Prussia", "Germany", ifelse(StateName == "Sardinia/Piedmont", "Italy", StateName))),
         cow_code = countrycode(StateName, "country.name", "cown"),
         year = StartYear1) %>%
  select(cow_code, year, StartYear1, EndYear1)

# get it into time-series format
war <- war[complete.cases(war$StartYear1),]
war_intra <- war_intra[complete.cases(war_intra$StartYr1),]

# remove years after 2010, or missing end year
war_intra <- war_intra[complete.cases(war_intra$EndYr1),]
war_intra <- war_intra[war_intra$StartYr1 < 2011,]


war <- war %>%
  arrange(cow_code, StartYear1) %>%
  mutate(atwar = map2(StartYear1, EndYear1, `:`)) %>%
  select(-year, -StartYear1, -EndYear1) %>%
  unnest()

war_intra <- war_intra %>%
  arrange(cow_code, StartYr1) %>%
  mutate(atwar_civil = map2(StartYr1, EndYr1, `:`)) %>%
  select(-year, -StartYr1, -EndYr1) %>%
  unnest()

war$year <- war$atwar
war_intra$year <- war_intra$atwar_civil
war$atwar <- 1
war_intra$atwar_civil <- 1

# remove missing country codes
war_intra <- war_intra[war_intra$cow_code != -8,]
...
war <- war %>% distinct()
war_intra <- war_intra %>% distinct()




war_ts <- left_join(A02_workinj, war, by = c("cow_code","year"))
war_ts <- left_join(war_ts, war_intra, by = c("cow_code","year"))

# change to iso3

war_ts$iso3 <- countrycode(war_ts$cow_code, "cown", "iso3c")
war_ts$iso3 <- ifelse(war_ts$cow_code == 345, "SRB", ifelse(war_ts$cow_code == 817, "VNM", war_ts$iso3))

war_ts <- select(war_ts, iso3, atwar, atwar_civil, year)
# unique(A02_workinj$cow_code) 165 OK!

# add zeros for not at war, make two years after war the time that matters (exclude the war itself)
war_ts <- war_ts %>%
arrange(iso3, year) %>%
  mutate(war = car::recode(atwar, recodes = "1 = 1;NA = 0"),
         war = ifelse(atwar_civil == 1, 1, war),
         atwar2 = lag(atwar),
         atwar3 = lag(atwar, 2L),
         atwar4 = lag(atwar, 3L),
         atwar5 = lag(atwar, 4L),
         atwar_n = ifelse(!is.na(atwar2), 1, ifelse(!is.na(atwar3), 1, ifelse(!is.na(atwar4), 1, ifelse(!is.na(atwar5), 1, 0)))),
         atwar = atwar_n,
         atwar2_civ = lag(atwar_civil),
         atwar3_civ = lag(atwar_civil, 2L),
         atwar4_civ = lag(atwar_civil, 3L),
         atwar5_civ = lag(atwar_civil, 4L),
         atwar_n_civ = ifelse(!is.na(atwar2_civ), 1, ifelse(!is.na(atwar3_civ), 1, ifelse(!is.na(atwar4_civ), 1, ifelse(!is.na(atwar5_civ), 1, 0)))),
         atwar_civil = atwar_n_civ)

# make a variable for any war & postwar time 
war_ts <- war_ts %>%
  mutate(war = ifelse(is.na(war), 0, war),
         atwar_any = atwar,
         atwar_any = ifelse(atwar_civil == 1, 1, atwar_any),
         atwar_any = ifelse(war == 1, 1, atwar_any))
...
covar_base <- merge(covar_base, war_ts, by=c("iso3","year"))
...
# Import unemployment data (from Carina 19.11.2020)

# original file name "UI_Laws_UploadWesis_incl Sources.xlsx" (in SeaFile)


unemp <- read.csv("data/A02_unemp.csv", header = T)
unemp <- select(unemp, cow_code, unemp_firstlaw)

A02_unemp <- left_join(A02_workinj, unemp, by = "cow_code")

# Drop missing countries
A02_unemp <- subset(A02_unemp, !is.na(unemp_firstlaw))

# Merge


A02_unemp <- A02_unemp %>%
  mutate(unemp_firstlaw = ifelse(unemp_firstlaw > 2010, NA, unemp_firstlaw)) # Must end at 2010 (network data limitation)

covar_ <- data.frame(covar_base)
head(covar_)

A02_2 <- data.frame(subset(  A02_unemp, select = c(iso3,unemp_firstlaw,year)  ))
...
diffnet_culture <- edgelist_to_diffnet(
  ...
  idvar    = "iso3",                  
  toavar   = "toa_unemp_A02",          # -- !! here just change time-of-adoption !! -- #
  timevar  = "year",
...
diffnet_colony[["expo_colony"]] <- exposure(diffnet_colony, valued = F)
...
# -- create time periods for stepwise PC Rate model, create 1880-1900 which we drop later because no countries adopt in this period
summary(diff_data_culture$t)
diff_data_culture$t0_20 <- ifelse(diff_data_culture$t < 21, 1, 0)
diff_data_culture$t21_49 <- ifelse(diff_data_culture$t >= 21 
...
d <- subset(  diffnet_colony_df, select = c(id,per,w_expo_colony, expo_colony)  )
...
save.image(file = "data/unemp.Rdata")
```
