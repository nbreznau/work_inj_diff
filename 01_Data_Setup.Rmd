...
war_ts <- select(war_ts, iso3, atwar, year)
...
war_ts <- war_ts %>%
arrange(iso3, year) %>%
  mutate(atwar2 = lag(atwar),
         atwar3 = lag(atwar, 2L),
         atwar_n = ifelse(!is.na(atwar2), 1, ifelse(!is.na(atwar3), 1, 0)),
         atwar = ifelse(atwar_n == 1, 1, atwar),
         atwar = ifelse(is.na(atwar), 0, atwar))
...
A02_unemp <- A02_unemp %>%
  mutate(unemp_firstlaw = ifelse(unemp_firstlaw < 1880, 1880, ifelse(unemp_firstlaw > 2010, NA, unemp_firstlaw))) # Must end at 2010 (network data limitation)
...
# -- create time periods for stepwise PC Rate model, use 33 for first group as the first law does not come into play until 1916
summary(diff_data_culture$t)
diff_data_culture$t0_33 <- ifelse(diff_data_culture$t < 33, 1, 0)
diff_data_culture$t33_49 <- ifelse(diff_data_culture$t >= 33 
