...
We run thse at 99% confidence but label it at 95, because in the robust clustered standard errors in Table 1, they only make the 95% CI cutoff (they are close). I haven't found an easy way to use robust se's in a coef_plot so this is the next best option
...
Of the 152 countries, `r sum(diff_data_ex$enacted)` enacted a first law and `r sum(diff_data_ex_socins$enacted)` enacted a first social insurance during the first 5 years of nation state formation.
...
ggplot(treat_plot, aes(formation, rev(n_id), fill = law_seq)) +
  ...
  values = c("Grey","chocolate4","seagreen4"),
