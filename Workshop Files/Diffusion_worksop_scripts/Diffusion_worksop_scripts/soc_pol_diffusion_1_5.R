...
temp <- tempfile()
download.file("http://www.barkhof.uni-bremen.de/~mwindzio/colonial_ties_edgelist.zip",temp)
colonial_ties_edgelist <- read.csv(unz(temp, "colonial_ties_edgelist.csv"))
unlink(temp)
...
# Introductions after 2010 (outside our time frame) need to be set to NA, otherwise NetdiffuseR has a problem
# Please insert your own TOA variable(s) instead of toa_ml_A06

covar$toa_ml_A06[covar$toa_ml_A06 > 2010] <- NA
